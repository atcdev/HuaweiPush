 ...
 * Examples:
 * ```
 * var API = require('HuaWeiPush');
 * var api = new API('client_id', 'client_secret');
 * ```
 * 以上即可满足单进程使用。
 ...
 * });
 * ```
 * @param {String} client_id 在华为开发者平台获取
 * @param {String} client_secret 在华为开发者平台获取
 * @param {Function} getToken 可选的。获取全局token对象的方法，多进程模式部署时需在意
 * @param {Function} saveToken 可选的。保存全局token对象的方法，多进程模式部署时需在意
 * @param {String} proxyOpt 可选的。代理
 */
var API = function (client_id, client_secret, getToken, saveToken, proxyOpt) {
  ...
  this.TOKENPRFEX = 'https://login.vmall.com/oauth2/token';
  this.SENDMESSAGEPrefix = 'https://api.vmall.com/rest.php';
  this.defaults = {};
};
...
API.prototype.request = function (url, opts, callback) {
  ...
  if(this.proxyOpt){
    ...
    options.proxy = this.proxyOpt;
    console.log(`-- HuaWeiPush urllib request in proxy: ${options.proxy}`);
  }
